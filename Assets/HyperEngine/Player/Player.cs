...
public class Player : MonoBehaviour {
    ...
    public KeyCode mapToggleKey;
    public KeyCode projectionChangeKey;
    ...
    void Update() {
        if (IsLocked()) {
            ...
            Vector2 lookXY = new Vector2(Input.GetAxis("Mouse X") * mouseSensitivity, Input.GetAxis("Mouse Y") * mouseSensitivity);
            if (UnityEngine.XR.XRSettings.enabled) {
                LookVRSnap(lookXY.x);
            } else {
            ...
            if (HM.K == 0.0f) {
                ...
                projCur = projNew;
                projInterp = projCur;
                if (Input.GetKeyDown(projectionChangeKey) && map && map.IsMapOut()) {
                    projTransition = 1.0f;
                    projNew += 1.0f;
                    if (projNew >= 2.0f) {
                        projNew = -1.0f;
            ...
            if (map && Input.GetKeyDown(mapToggleKey)) {
        ...
        if (!IsLocked()) {
            ...
            Vector2 dxy = new Vector2(Input.GetAxis("Horizontal"), Input.GetAxis("Vertical"));
            ...
            if (freeFly) {
                ...
                //Jump if allowable
                if (velocity.y <= 0.0f && isRecentGrounded && Input.GetKeyDown(KeyCode.Space) && !IsLocked() && walkingSpeed > 0.0f) {
